%
%  Thesis Vorlage für die Hochschule Heilbronn
%
%  Created by Prof. Dr. Detlef Stern on 2010-08-14.
%  Updated by Valentin Weber on 2020-10-05.
%  Copyright (c) 2020 . All rights reserved.
%
\documentclass[12pt,toc=bib,toc=listof]{scrreprt}
\usepackage[ngerman]{babel} 
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage{setspace}
\usepackage{geometry}

\usepackage{hyperref}
\hypersetup{
  ,colorlinks=true
  ,linkcolor=blue
  ,citecolor=blue
  ,filecolor=blue
  ,urlcolor=blue
  }

% Fachbezogene Werte (müssen aktualisiert werden)
\newcommand{\hhnsubject}{Bachelor Thesis}
\newcommand{\hhnlecturer}{Prof. Dr. Tim Reichert}
\newcommand{\hhnsecondlecturer}{Ulrich Straus}

% Vom Studierenden zu aendernde Werte
\newcommand{\reprttopic}{Entwicklung eines physikbasierten Charaktercontrollers mit Unity Machine Learning}
\newcommand{\reprtstudentname}{Simon Grözinger}
\newcommand{\reprtstudentid}{205047}
\urldef{\reprtstudentmail}\url{sgroezin@stud.hs-heilbronn.de}

\usepackage{ifpdf}
\ifpdf
\usepackage[pdftex]{graphicx}
\else
\usepackage{graphicx}
\fi

\usepackage[headsepline]{scrlayer-scrpage}
\pagestyle{scrheadings}
\clearscrheadfoot
\ihead{\hhnsubject: \reprttopic}
\ohead{\pagemark}
\renewcommand*{\chapterpagestyle}{scrheadings}
\renewcommand*{\chapterheadstartvskip}{}

% ============= BibLatex =============
\usepackage[backend=bibtex,style=numeric]{biblatex}
%Literaturverzeichnis
\addbibresource{Literatur.bib}

% Extra Packages
\usepackage{float}
\usepackage[table,svgnames]{xcolor}
\usepackage{listings}
\lstset{
language=[Sharp]C,
captionpos=b,
numbers=left, %Nummerierung
frame=single, % Oberhalb und unterhalb des Listings ist eine Linie
showspaces=false,
showtabs=false,
breaklines=true,
showstringspaces=false,
breakatwhitespace=true,
escapeinside={(*@}{@*)},
commentstyle=\color{greencomments},
morekeywords={partial, var, value, get, set},
keywordstyle=\color{bluekeywords},
stringstyle=\color{redstrings},
basicstyle=\ttfamily\small,
literate=%
  {Ö}{{\"O}}1
  {Ä}{{\"A}}1
  {Ü}{{\"U}}1
  {ß}{{\ss}}1
  {ü}{{\"u}}1
  {ö}{{\"o}}1
  {ä}{{\"a}}1,
  aboveskip=2em,
  belowskip=1.5em
  }
\lstdefinelanguage{diff}{
    basicstyle=\ttfamily\small,
    morecomment=[f][\color{diffstart}]{@@},
    morecomment=[f][\color{diffincl}]{+},
    morecomment=[f][\color{diffrem}]{-},
  }
\usepackage{tikz}
\usetikzlibrary{shapes.geometric, arrows, fit}
\pgfdeclarelayer{bg}
\pgfsetlayers{bg,main}
\tikzstyle{rounded} = [rectangle, rounded corners, minimum width=3cm, minimum height=1cm, text centered, draw=black]
\tikzstyle{decision} = [diamond, aspect=2, minimum width=2cm, minimum height=2cm, text centered, draw=black]
\tikzstyle{uni} = [-latex, line width=0.3mm]
\tikzstyle{bi} = [latex-latex, line width=0.3mm]
\usepackage{soul}
\usepackage{subcaption}
\usepackage{longtable}
  
\definecolor{bluekeywords}{rgb}{0,0,1}
\definecolor{greencomments}{rgb}{0,0.5,0}
\definecolor{redstrings}{rgb}{0.64,0.08,0.08}
\definecolor{xmlcomments}{rgb}{0.5,0.5,0.5}
\definecolor{types}{rgb}{0.17,0.57,0.68}
\definecolor{diffstart}{named}{Grey}
\definecolor{diffincl}{named}{Green}
\definecolor{diffrem}{named}{OrangeRed}

%table id counter
\newcounter{rowids}
\newcommand\rowids{\stepcounter{rowids}\arabic{rowids}}
\newcommand\rowidsclear{\setcounter{rowids}{0}}

% Deckblatt Definitionen (begin)
\titlehead{\flushright\includegraphics[scale=0.1]{./img/hhn.png}}
\subject{{\hhnsubject{}}}
\title{\reprttopic}
\author{\reprtstudentname\footnote{\reprtstudentid, \reprtstudentmail}}
%% Datum nie auf einen festen Wert setzen
\publishers{Erstprüfer: \hhnlecturer \\
Zweitprüfer: \hhnsecondlecturer \\
}
% Deckblatt Definitionen (end)

\begin{document}
\pagenumbering{Roman} 
\selectlanguage{ngerman}
\maketitle
\newgeometry{left=30mm, top=25mm, right=15mm, bottom=25mm}

\input{eidesstattliche_erklärung}

\tableofcontents

%Abbildungsverzeichnis
\listoffigures

%Tabellenverzeichnis
\listoftables

%Codesnippet-verzeichnis
\lstlistoflistings

\newpage
\pagenumbering{arabic}

\input{01_einleitung}
\input{02_grundlagen}
\input{03_analyse}
\input{04_charaktercontroller}
\input{05_zusammenfassung_ausblick}

%Literaturverzeichnis
\printbibliography[title=Literaturverzeichnis]

\newpage
\appendix
\input{a_training_doc}
\input{b_codeausschnitte}

\end{document}